...
function toGatewayWalletRow(data) {
  return {
    page_id: data.pageId,
    owner_id: data.ownerId,
    page_username: data.pageUsername,
...
export async function getGatewayWalletForPage(pageId) {
  const supabase = createSupabaseAdminClient()
  const { data, error } = await supabase
    .from('quid_gateway_wallets')
    .select('*')
    .eq('page_id', pageId)
    .maybeSingle()

  if (error) {
    if (error.message.includes('quid_gateway_wallets')) {
      return null
    }
  ...
  return toGatewayWallet(data)
}

export async function upsertGatewayWalletRecord(data) {
  const supabase = createSupabaseAdminClient()
  const { data: row, error } = await supabase
    .from('quid_gateway_wallets')
    .upsert(toGatewayWalletRow(data), { onConflict: 'page_id' })
    .select('*')
    .single()
...
export async function getLatestCheckoutForOwner(ownerId) {
  const supabase = createSupabaseAdminClient()
  const { data, error } = await supabase
    .from('quid_payments')
    .select('*')
...
function isMeaningfulActivitySource(source, txHash) {
  const normalizedSource = String(source ?? '').toLowerCase()
  const normalizedHash = String(txHash ?? '').toLowerCase()

  return (
    !normalizedSource.includes('balance sync') &&
    !normalizedSource.includes('unindexed') &&
    !normalizedSource.includes('source not identified') &&
    !normalizedHash.startsWith('balance-sync-')
  )
}

export async function getRecentActivityForOwner(ownerId) {
  const supabase = createSupabaseAdminClient()
  const [paymentResult, walletActivityResult] = await Promise.all([
    supabase
      .from('quid_payments')
      .select('created_at')
      .eq('owner_id', ownerId)
    ...
    supabase
      ...
      .select('happened_at, source, tx_hash')
      .eq('owner_id', ownerId)
      .order('happened_at', { ascending: false })
      .limit(20)
  ])

  if (paymentResult.error && !paymentResult.error.message.includes('quid_payments')) {
    throw new Error(paymentResult.error.message)
  }

  if (walletActivityResult.error && !walletActivityResult.error.message.includes('quid_wallet_activity')) {
    throw new Error(walletActivityResult.error.message)
  }

  const latestPaymentAt = paymentResult.data?.created_at ?? null
  const latestWalletActivityAt = (walletActivityResult.data ?? [])
    .find((activity) => isMeaningfulActivitySource(activity.source, activity.tx_hash))
    ?.happened_at ?? null
  const timestamps = [latestPaymentAt, latestWalletActivityAt].filter(Boolean)
  ...
  return {
    occurredAt: timestamps.sort((left, right) => Date.parse(right) - Date.parse(left))[0]
  }
}

export async function getPaymentSummaryForOwner(ownerId) {
  const [payments, latestCheckout, recentActivity] = await Promise.all([
    listPaymentsForOwner(ownerId, 100),
    getLatestCheckoutForOwner(ownerId),
    getRecentActivityForOwner(ownerId)
  ])
  ...
  return {
    ...
    latestPaymentAt: latestCheckout?.createdAt ?? null,
    recentActivityAt: recentActivity?.occurredAt ?? latestCheckout?.createdAt ?? null
  }
}
