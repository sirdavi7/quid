...
function isCompletedCircleTransaction(state) {
  return ['COMPLETE', 'COMPLETED', 'CONFIRMED'].includes(String(state ?? '').toUpperCase())
}
...
async function waitForCircleTransactionCompletion(client, transactionId, label) {
  let latest = null

  for (let attempt = 0; attempt < 20; attempt += 1) {
    const response = await client.getTransaction({ id: transactionId })
    latest = normalizeCircleTransactionPayload(response.data)
...
export async function depositCircleWalletUsdcToGateway({ walletId, chainId, usdcAddress, amount }) {
  const client = createCircleWalletsClient()
  ...
  const approval = normalizeCircleTransactionPayload(approvalResponse.data)

  if (!approval.id) {
    throw new Error('Circle did not return an approval transaction for this Gateway deposit.')
  }

  await waitForCircleTransactionCompletion(client, approval.id, 'USDC approval')
  ...
  const created = normalizeCircleTransactionPayload(depositResponse.data)

  if (!created.id) {
    throw new Error('Circle did not return a Gateway deposit transaction.')
  }

  const resolved = await waitForCircleTransactionCompletion(client, created.id, 'Gateway deposit')
  const txHash = resolved.txHash ?? created.txHash
