export function getFriendlyAuthError(message) {
  ...
  if (
    text.includes('signups not allowed for otp') ||
    text.includes('user not found') ||
    text.includes('email not found')
  ) {
    return 'This email is not registered on Quid. Create an account first.'
  }
  ...
  return 'We could not complete that sign-in step. Check your details and try again.'
}
